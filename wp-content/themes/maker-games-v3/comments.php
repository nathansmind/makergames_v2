<div id="comments">
	...
	<?php if($comments) : ?>
		<h5><?php comments_number('No comments', 'One comment', '% comments'); ?></h5>
	    <ol>
		        ...
		        <li id="comment-<?php comment_ID(); ?>" class="comment <?php if($i&1) { echo 'odd';} else {echo 'even';} ?> <?php $user_info = get_userdata(1); if ($user_info->ID == $comment->user_id) echo 'authorComment'; ?> <?php if ($comment->user_id > 0) echo 'user-comment'; ?>">
		            <?php if ($comment->comment_approved == '0') : ?> <!-- if comment is awaiting approval -->
		                <p class="waiting-for-approval">
		                	<em class="thanks">Thanks, Your comment is awaiting approval.</em>
		                </p>
		            <?php endif; ?>
		            <div class="comment-meta">
		            	<p class="gravatar"><?php if(function_exists('get_avatar')) { echo get_avatar($comment, '36'); } ?></p>
		            	<span class="name"><?php comment_type(); ?> by <?php comment_author_link(); ?> on <?php comment_date(); ?></span><br>
		            </div><!--.commentMeta-->
		    ...
		    <?php if(get_option('comment_registration') && !$user_ID) : ?>
		        ...
		        <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
		            ...
		            <?php else : ?>
		                ...
		                <p>
		                	<label for="email">Email <span class="will-not">(will not be published)<?php if($req) echo "*"; ?></small></label><br>
		            ...
		            <p>
		            	<input name="submit" type="submit" id="submit" tabindex="5" value="Submit" class="button radius" />
