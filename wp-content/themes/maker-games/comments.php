	    	...
	    	<?php if($comment_type == 'comment') { ?> <!-- outputs only comments -->
		        <li id="comment-<?php comment_ID(); ?>" class="comment <?php if($i&1) { echo 'odd';} else {echo 'even';} ?> <?php $user_info = get_userdata(1); if ($user_info->ID == $comment->user_id) echo 'authorComment'; ?> <?php if ($comment->user_id > 0) echo 'user-comment'; ?>">
		            ...
		            <div class="comment-meta clear">
		            	<p class="gravatar left"><?php if(function_exists('get_avatar')) { echo get_avatar($comment, '36'); } ?></p>
		            	<div>
			            	<?php edit_comment_link('Edit Comment', '', ''); ?>
			            	<?php comment_type(); ?> by <?php comment_author_link(); ?> on <span class="date"><?php comment_date(); ?></span>
		            	</div>
		            </div><!--.commentMeta-->
		        </li>
		...
		<?php if(comments_open()) : ?>
		    <?php if(get_option('comment_registration') && !$user_ID) : ?>
		        ...
		        <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
		            ...
		            <?php else : ?>
		                <p>
							<label for="author">Name <span class="required"><?php if($req) echo "*"; ?></span></label>
							<input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="22" tabindex="1" />
		                </p>
		                <p>
		                	<label for="email">Email (will not be published) <span class="required"><?php if($req) echo "*"; ?></span></label>
		            ...
		            <?php endif; ?>
		            <p>
		            	<label for="comment">Comment <span class="required"><?php if($req) echo "*"; ?></span></label>
		            	<textarea name="comment" id="comment" cols="100%" rows="10" tabindex="4"></textarea>
		            </p>
		            <p>
		            	<input class="button radius" name="submit" type="submit" id="submit" tabindex="5" value="Submit Comment" />
